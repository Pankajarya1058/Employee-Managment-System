...
const AdminSummary = () => {
  ...
  return (
    <div className='p-6'>
      ...
      <div className='grid grid-cols-3 md:grid-col-3 gap-4 mt-6'>
        <SummaryCard icon={<FaUsers />} text="Total Employees" number={summary.totalEmployees} color="bg-teal-600" />
        <SummaryCard icon={<FaBuilding />} text="Total Departments" number={summary.totalDepartments} color="bg-gray-500" />
        <SummaryCard icon={<FaMoneyBillWave />} text="Monthly Pay" number={summary.totalSalary} color="bg-emerald-500" />
      </div>
      <div>
        <div className='mt-12'>
          <h4 className='text-2xl font-bold'>Leave Details</h4>
          <div className='grid grid-cols-4 md:grid-cols-2 gap-6 mt-6'>
